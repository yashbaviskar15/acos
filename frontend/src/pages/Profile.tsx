...
export const Profile: React.FC<ProfileProps> = ({ user, onNavigateToBilling }) => {
  ...
  const fetchProfile = async () => {
    try {
      if (token) {
        const data = await apiFetch<ProfileResponse>('/v1/auth/me', { headers: { 'Authorization': `Bearer ${token}` } });
        setProfileData(data);
  ...
  const handleRoleChange = async (newRole: string) => {
    ...
    try {
      const data = await apiFetch<RoleUpdateResponse>('/v1/auth/role/update', {
        method: 'POST',
      ...
      fetchProfile();
      const savedUser = JSON.parse(localStorage.getItem('arvanta_user') || '{}');
      savedUser.role = data.role;
      localStorage.setItem('arvanta_user', JSON.stringify(savedUser));
    } catch {
  ...
  const fetchMfaSetup = async () => {
    try {
      const data = await apiFetch<MfaSetupResponse>('/v1/auth/mfa/setup', {
        method: 'POST',
  ...
  const handleEnableMfa = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiFetch<MfaSetupResponse>('/v1/auth/mfa/enable', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
        body: JSON.stringify({ mfa_code: mfaTestCode }),
      });
      setMfaMsg({ type: 'success', text: data.message ?? 'MFA enabled successfully.' });
      setMfaTestCode('');
      fetchProfile();
    } catch {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-[#0F2038] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-sm relative overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white font-black text-2xl sm:text-3xl flex items-center justify-center shadow-lg shadow-blue-600/25 border-2 border-white/20">
              {displayName.charAt(0).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">{displayName}</h2>
                <span className="px-3 py-1 rounded-full text-[11px] font-mono font-black bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 uppercase">
                  {role}
                </span>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 font-mono">{email}</p>
              <div className="flex items-center gap-2 mt-1">
                <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">Account ID: <span className="text-slate-900 dark:text-white font-bold">{accountId}</span></p>
                <button onClick={handleCopyAccountId} className="text-slate-400 hover:text-blue-600 transition-colors cursor-pointer" title="Copy Account ID">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex items-center gap-4 shadow-sm w-full sm:w-auto">
            <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 flex items-center justify-center font-bold shrink-0">
              <Clock className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">10-Day Free Trial</p>
              <p className="text-sm font-black text-amber-700 dark:text-amber-400">{trialDaysRemaining > 0 ? `${trialDaysRemaining} Days Remaining` : 'Trial Expired'}</p>
            </div>
            <button
              onClick={onNavigateToBilling}
              className="ml-auto sm:ml-4 px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-black font-extrabold text-xs rounded-xl shadow-md transition-all cursor-pointer shrink-0"
            >
              Upgrade Plan
            </button>
          </div>
        ...
        <div className="flex items-center gap-2 border-t border-slate-200 dark:border-slate-800/80 pt-6 mt-6 overflow-x-auto">
          {[
