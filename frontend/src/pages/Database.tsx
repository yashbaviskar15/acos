...
export const Databases: React.FC<DatabaseProps> = ({ token }) => {
  ...
  const fetchDatabases = async () => {
    setLoading(true);
    try {
      const data = await apiFetch<any[]>('/v1/databases/instances', { token }).catch(() => null);
      if (data) setDatabases(Array.isArray(data) ? data : []);

      const sum = await apiFetch<any>('/v1/databases/summary', { token }).catch(() => null);
