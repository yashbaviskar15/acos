...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess, onGoToLanding, initialTab = 'signin' }) => {
  ...
  const handleMfaVerify = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await apiFetch<AuthResponse>('/v1/auth/mfa/verify', {
        method: 'POST',
        body: JSON.stringify({ email: mfaUserData?.email || email, mfa_code: mfaCode }),
      });

      if (!data.access_token) {
        throw new Error('Missing access token from MFA verification response.');
      }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await apiFetch<RegisterResponse>('/v1/auth/register', {
        method: 'POST',
