...
import { 
  ...
  ArrowUpDown, ArrowUp, ArrowDown, X, Terminal, Copy, Check, ShieldAlert
} from 'lucide-react';
...
export const Compute: React.FC<ComputeProps> = ({ token }) => {
  ...
  const [connectTarget, setConnectTarget] = useState<any | null>(null);
  const [connectInfo, setConnectInfo] = useState<any | null>(null);
  const [copiedCmd, setCopiedCmd] = useState(false);

  const handleOpenConnect = async (inst: any) => {
    setConnectTarget(inst);
    setConnectInfo(null);
    try {
      const data = await apiFetch<any>(`/v1/compute/instances/${inst.id}/connect`, { token });
      setConnectInfo(data);
    } catch {
      setConnectInfo({ connectable: false, reason: 'Failed to retrieve connection instructions.' });
    }
  };
  ...
  return (
    <div className="space-y-5 w-full font-sans pb-10">
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3.5">
          ...
          <div className="bg-white dark:bg-[#0F2038] border border-slate-200 dark:border-slate-800 p-4 sm:p-5 rounded-2xl shadow-xs">
            ...
            <p className="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1 font-mono">{summary.running || instances.filter((i: any) => i.status === 'RUNNING').length}</p>
          </div>
          <div className="bg-white dark:bg-[#0F2038] border border-slate-200 dark:border-slate-800 p-4 sm:p-5 rounded-2xl shadow-xs">
            <span className="text-[10px] font-mono uppercase font-bold text-slate-500 dark:text-slate-400">Stopped</span>
            <p className="text-2xl font-black text-slate-500 dark:text-slate-400 mt-1 font-mono">{summary.stopped || instances.filter((i: any) => i.status === 'STOPPED').length}</p>
          </div>
          <div className="bg-white dark:bg-[#0F2038] border border-slate-200 dark:border-slate-800 p-4 sm:p-5 rounded-2xl shadow-xs">
              ...
              {loading && instances.length === 0 ? (
                ...
                paginatedInstances.map((inst: any) => (
                  <tr key={inst.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="p-4 text-slate-700 dark:text-slate-300">{inst.private_ip || '—'}</td>
                    ...
                    <td className="p-4 text-right space-x-1 shrink-0">
                      <button
                        ...
                        className="p-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-lg transition-colors cursor-pointer"
      ...
      {/* Real SSH Connection Guide Modal */}
      <ModalPortal isOpen={!!connectTarget} onClose={() => setConnectTarget(null)}>
        {connectTarget && (
          <div className="space-y-4 text-xs font-mono">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Terminal className="w-4 h-4 text-blue-500" />
                Connect to {connectTarget.name}
              </h3>
              <button onClick={() => setConnectTarget(null)} className="text-slate-400 hover:text-white font-bold text-base cursor-pointer">×</button>
            </div>

            {connectInfo?.connectable ? (
              <div className="space-y-3">
                <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl space-y-1">
                  <div className="flex items-center gap-1.5 text-emerald-800 dark:text-emerald-300 font-bold text-[11px]">
                    <span>Public IP Reachable: {connectInfo.public_ip}</span>
                  </div>
                  <p className="text-[10px] text-slate-600 dark:text-slate-400">
                    Use your SSH private key file to establish a secure encrypted shell session to the instance.
                  </p>
                </div>

                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-slate-400 uppercase font-bold">SSH Command</span>
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(connectInfo.ssh_command);
                        setCopiedCmd(true);
                        setTimeout(() => setCopiedCmd(false), 2000);
                      }}
                      className="px-2 py-0.5 bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 rounded text-[10px] font-bold flex items-center gap-1 cursor-pointer"
                    >
                      {copiedCmd ? <Check className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3" />}
                      Copy Command
                    </button>
                  </div>
                  <pre className="p-3 bg-black text-emerald-400 rounded-xl text-[11px] overflow-x-auto border border-slate-800 font-mono">
              ...
              <div className="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl space-y-2 font-sans">
                <div className="flex items-center gap-2 text-amber-800 dark:text-amber-400 font-bold text-xs">
                  ...
                  <span>Instance Awaiting Provider Assignment</span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                  {connectInfo?.reason || "This virtual machine does not have a live public IP address assigned yet. To provision a real hypervisor VM, configure an AWS, GCP, Azure, or Docker provider in Settings → Cloud Providers."}
                </p>
              </div>
            )}
